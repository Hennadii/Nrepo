...
namespace Nrepo.DataAccess.EntityFramework.Testing.FKSoftDeletableAuditableTesting
{
	...
	public partial class EFRepositoryTesting
	{
		...
		private void AssertUsingEntities()
		{
			...
			Assert.AreEqual(false, directors[0].IsDeleted);
			...
			Assert.AreEqual(false, directorTrackers[0].IsDeleted);
		}
		...
		private void AssertOffices()
		{
			...
			Assert.AreEqual(false, offices[0].IsDeleted);
			...
			Assert.AreEqual(false, offices[1].IsDeleted);
		}
		...
		private void AssertCars()
		{
			...
			Assert.AreEqual(false, cars[0].IsDeleted);
			...
			Assert.AreEqual(false, cars[1].IsDeleted);
		}
		...
		private void AssertProjects(long? project1ManagerId, long? project2ManagerId)
		{
			...
			Assert.AreEqual(false, projects[0].IsDeleted);
			...
			Assert.AreEqual(false, projects[1].IsDeleted);
		...
		private void AssertComputers(DateTime creationDate, DateTime updatingDate, string data, bool isDeleted)
		{
			...
			Assert.AreEqual(isDeleted, computers[0].IsDeleted);
		...
		private void AssertDescriptions(DateTime creationDate, DateTime updatingDate, string data, bool isDeleted)
		{
			...
			Assert.AreEqual(isDeleted, descriptions[0].IsDeleted);
		}

		private void AssertThing(Thing thing, DateTime creationDate,
			DateTime updatingDate, string data, long managerId, bool isDeleted)
		{
			...
			Assert.AreEqual(isDeleted, thing.IsDeleted);
